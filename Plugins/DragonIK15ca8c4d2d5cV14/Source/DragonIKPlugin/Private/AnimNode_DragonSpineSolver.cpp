...
void FAnimNode_DragonSpineSolver::GetResetedPoseInfo(FCSPose<FCompactPose>& MeshBases)
{
	...
	{
		if (MeshBases.GetPose().IsValidIndex(combined_indices[i]))
...
void FAnimNode_DragonSpineSolver::GetAnimatedPoseInfo(FCSPose<FCompactPose>& MeshBases, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if ((owning_skel->GetWorld()->WorldType != EWorldType::EditorPreview || Work_Outside_PIE) && test_counter > 4)
	{
		...
		bool Check_Root_Second_Same = false;

		if(combined_indices.Num() > 2 && MeshBases.GetPose().IsValidIndex(combined_indices[1]) && MeshBases.GetPose().IsValidIndex(combined_indices[0]))
		{

			FTransform updated_transform_0 = MeshBases.GetComponentSpaceTransform(combined_indices[0]);
			FTransform updated_transform_1 = MeshBases.GetComponentSpaceTransform(combined_indices[1]);
		
			if ( (updated_transform_0.GetLocation() - updated_transform_1.GetLocation()).Size() < 0.000001f )
				Check_Root_Second_Same = true;
		}
		...
		for (int i = 0; i < NumTransforms - 0; i++)
		{
			...
			if ((i < (NumTransforms - tip_reduction)) && (i < Total_spine_alphas.Num()) && MeshBases.GetPose().IsValidIndex(combined_indices[i]) )
			{
				...
				else
				{
					...
					//	if (i == 1 && i != NumTransforms - 1 && Check_Root_Second_Same == false)
						if (i == 1 && i != NumTransforms - 1 )
						{
							if (Check_Root_Second_Same == false)
							{
						...
						if(use_type2_input)
						{
							if(i > 1 && i < NumTransforms - 1)
							{
								...
								FTransform Original_Pelvis_Transform_WS = Original_Pelvis_Transform*owner_skel_w_transform;
								Original_Pelvis_Transform_WS.SetRotation(owner_skel_w_transform.GetRotation());
								
								
								FTransform Original_Current_Transform_WS = MeshBases.GetComponentSpaceTransform(combined_indices[i])*owner_skel_w_transform;
								...
								FQuat diff_rot = FQuat::FindBetweenNormals(body_to_chest_orig_look,body_to_chest_look);
								FTransform Diff_Rot_Transform = Original_Pelvis_Transform_WS;

								Diff_Rot_Transform.SetRotation(Diff_Rot_Transform.GetRotation()*diff_rot);

								FTransform Original_Current_Transform_RelPelvis =  Original_Current_Transform_WS*Original_Pelvis_Transform_WS.Inverse();

								updated_transform =  (Original_Current_Transform_RelPelvis*(Diff_Rot_Transform))*owner_skel_w_transform.Inverse();

								FVector Pelvis_Change_Difference = (AnimatedBoneTransforms[0].Transform.GetLocation() - Original_Pelvis_Transform.GetLocation());

								updated_transform.AddToTranslation(Pelvis_Change_Difference);
...
void FAnimNode_DragonSpineSolver::UpdateInternal(const FAnimationUpdateContext& Context)
{
	...
	if (Character_Actor)
	{
		Dragon_Trace_Manager = Context.AnimInstanceProxy->GetSkelMeshComponent()->GetOwner()->FindComponentByClass<UDragonIKTraceManagerComponent>();
		...
		delta_seconds_saved = Context.AnimInstanceProxy->GetSkelMeshComponent()->GetWorld()->DeltaTimeSeconds * Character_Actor->CustomTimeDilation;

		solver_time_reference += Context.AnimInstanceProxy->GetSkelMeshComponent()->GetWorld()->DeltaTimeSeconds;
	}
	else
	{
		if(Context.AnimInstanceProxy->GetSkelMeshComponent()!=nullptr)
				...
				//	FTransform Temp_Diff_Transform = Temp_Zero_Transform* owner_skel_w_transform.Inverse();
					...
					//	line_trace_func(*owning_skel, feet_centre_point + character_direction_vector*Temp_Duck_Pelvis_Offset, feet_centre_point + (character_direction_vector * Pelvis_Crouch_Height) + character_direction_vector*Temp_Duck_Pelvis_Offset, Duck_Pelvis_Hit, spine_Feet_pair[i].Spine_Involved.BoneName, spine_Feet_pair[i].Spine_Involved.BoneName, Duck_Pelvis_Hit, FLinearColor::White, false, true);
						...
						else
						{
							...
							Duck_Pelvis_Point = UKismetMathLibrary::VInterpTo(Duck_Pelvis_Point, Duck_Pelvis_Highest_Point, 1 - FMath::Exp(-Smooth_Factor * Context.AnimInstanceProxy->GetSkelMeshComponent()->GetWorld()->GetDeltaSeconds()), Formatted_Trace_Lerp * 0.1f);
					...
					else
					{
						...
						if (Duck_Chest_Hit.bBlockingHit)
						{
							...
							else
							{
								...
								Duck_Chest_Point = UKismetMathLibrary::VInterpTo(Duck_Chest_Point, Duck_Chest_Hit.ImpactPoint, 1 - FMath::Exp(-Smooth_Factor * Context.AnimInstanceProxy->GetSkelMeshComponent()->GetWorld()->GetDeltaSeconds()), Formatted_Trace_Lerp * 0.1f);
							}
						...
						else
						{
							...
							Duck_Chest_Point = UKismetMathLibrary::VInterpTo(Duck_Chest_Point, Duck_Chest_Highest_Point, 1 - FMath::Exp(-Smooth_Factor * Context.AnimInstanceProxy->GetSkelMeshComponent()->GetWorld()->GetDeltaSeconds()), Formatted_Trace_Lerp * 0.1f);
...
{
	//GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " combined_indices : " + FString::SanitizeFloat( combined_indices.Num()) );

	/*
	for (int i=0; i < combined_indices.Num(); i++)
	{

		FName Bone_Name = owning_skel->GetBoneName(combined_indices[i].GetInt());
		FCompactPoseBoneIndex C_Bone_Index = FCompactPoseBoneIndex(owning_skel->GetBoneIndex(Bone_Name));


		GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " INDICE : " + FString::SanitizeFloat(C_Bone_Index.GetInt()));

		if (Output.Pose.GetPose().IsValidIndex(C_Bone_Index))
		{
			Output.Pose.GetComponentSpaceTransform(C_Bone_Index);
		}

		//FBoneTransform(C_Bone_Index, Output.Pose.GetComponentSpaceTransform(C_Bone_Index));
...
void FAnimNode_DragonSpineSolver::CoreEvaluation(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if (solve_should_fail == false && (FVector(0, 0, 0) - Output.AnimInstanceProxy->GetActorTransform().GetScale3D()).Size() > 0 && spine_Feet_pair.Num() > 0 && FAnimWeight::IsRelevant(ActualAlpha) && IsValidToEvaluate(Output.AnimInstanceProxy->GetSkeleton(), Output.AnimInstanceProxy->GetRequiredBones()) && Output.ContainsNaN() == false)
	{
		...
		if (is_snake)
		{
			...
			{
				if ( Output.Pose.GetPose().IsValidIndex(combined_indices[i]) && combined_indices.Num() > 0 && spine_between_transforms.Num() > 0 && (i-1) < spine_between_transforms.Num())
				{
					...
					if (use_custom_fixed_bone_height && is_snake)
					{
						
							if (i < custom_fixed_bone_height.snake_bone_height_array.Num())
							{
								chest_distance = custom_fixed_bone_height.snake_bone_height_array[i];
							}						
					}
		...
		//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, "spine_Feet_pair "+FString::SanitizeFloat(spine_Feet_pair.Num()) + " ---- "+FString::SanitizeFloat(Total_spine_heights.Num()));
			...
			for (int i = 0; i < spine_Feet_pair.Num(); i++)
			{
				...
				if (use_custom_fixed_bone_height && is_snake)
				{
					if (i == 0)
					{
						if (custom_fixed_bone_height.snake_bone_height_array.Num() > 0)
						{
							chest_distance = custom_fixed_bone_height.snake_bone_height_array[0];
						}

					}
					else if(i == spine_Feet_pair.Num()-1)
					{

						if (custom_fixed_bone_height.snake_bone_height_array.Num() > 0)
						{
							chest_distance = custom_fixed_bone_height.snake_bone_height_array[Total_spine_heights.Num() - 1];
						}
...
bool FAnimNode_DragonSpineSolver::IsValidToEvaluate(const USkeleton* Skeleton, const FBoneContainer& RequiredBones)
{
	...
	for (int i = 0; i < spine_Feet_pair.Num(); i++)
	{
		...
		if (spine_Feet_pair[i].Spine_Involved.IsValidToEvaluate(RequiredBones) == false)
		{
			feet_is_true = false;
		}
		...
		for (int j = 0; j < spine_Feet_pair[i].Associated_Feet.Num(); j++)
		{
			...
			if (spine_Feet_pair[i].Associated_Feet[j].IsValidToEvaluate(RequiredBones) == false )
			{
				feet_is_true = false;
			}
		...
		for (int j = 0; j < spine_Feet_pair[i].Associated_Knees.Num(); j++)
		{

			if (spine_Feet_pair[i].Associated_Knees[j].IsValidToEvaluate(RequiredBones) == false)
			{
				feet_is_true = false;
			}
		...
		for (int j = 0; j < spine_Feet_pair[i].Associated_Thighs.Num(); j++)
		{

			if (spine_Feet_pair[i].Associated_Thighs[j].IsValidToEvaluate(RequiredBones) == false)
			{
				feet_is_true = false;
			}
	...
	if(use_type2_input)
	{
		...
		for (int i = 0; i < dragon_input_type2_data.Spine_Structure.Num(); i++)
		{
			...
			if(dragon_bone_data.Start_Spine.IsValidToEvaluate(RequiredBones) == false || dragon_bone_data.Pelvis.IsValidToEvaluate(RequiredBones) == false)
			{
				solve_should_fail = true;
			}
		...
		return (RequiredBones.IsValid() && solve_should_fail == false && feet_is_true && dragon_bone_data.Start_Spine.IsValidToEvaluate(RequiredBones) &&
	...
	else
	{
		...
		if(!RequiredBones.BoneIsChildOf(dragon_bone_data.Start_Spine.BoneIndex, dragon_bone_data.Pelvis.BoneIndex))
			solve_should_fail = true;
	...
	//	Pelvis_ref.IsValidToEvaluate(RequiredBones));
		...
		return (RequiredBones.IsValid() && solve_should_fail == false && feet_is_true && dragon_input_data.FeetBones.Num() % 2 == 0 &&
			feet_is_empty == false && dragon_bone_data.Start_Spine.IsValidToEvaluate(RequiredBones) &&
			dragon_bone_data.Pelvis.IsValidToEvaluate(RequiredBones));
	...
	//if (spine_Feet_pair.Num() == 0)
	{
		...
		if (allow_initialization_flow)
		{
			...
			if (solve_should_fail == false)
			{
				...
				//GEngine->AddOnScreenDebugMessage(-1, 10.1f, FColor::Red, " BONE_ROOT = " + owning_skel->GetBoneName(spine_Feet_pair[0].Spine_Involved.BoneIndex).ToString());
				...
				if (spine_Feet_pair.Num() > 1)
				{
					...
					//	FCompactPoseBoneIndex BoneIndex = spine_Feet_pair[spine_Feet_pair.Num() - 1].Spine_Involved.GetCompactPoseIndex(*SavedBoneContainer);
						...
						const FCompactPoseBoneIndex RootIndex = FCompactPoseBoneIndex(spine_Feet_pair[0].Spine_Involved.BoneIndex);
						FCompactPoseBoneIndex BoneIndex = FCompactPoseBoneIndex(spine_Feet_pair[spine_Feet_pair.Num() - 1].Spine_Involved.BoneIndex);


						int while_counter = 0;
					//	GEngine->AddOnScreenDebugMessage(-1, 10.1f, FColor::Red, " BONE_ROOT = " + owning_skel->GetBoneName(RootIndex.GetInt()).ToString());
					//	GEngine->AddOnScreenDebugMessage(-1, 10.1f, FColor::Red, " BONE_TIP = " + owning_skel->GetBoneName(BoneIndex.GetInt()).ToString());

					//	GEngine->AddOnScreenDebugMessage(-1, 10.1f, FColor::Red, " BONE_ROOT = " + owning_skel->GetBoneName(spine_Feet_pair[0].Spine_Involved.CachedCompactPoseIndex.GetInt()).ToString());
					//	GEngine->AddOnScreenDebugMessage(-1, 10.1f, FColor::Red, " BONE_TIP = " + owning_skel->GetBoneName(spine_Feet_pair[spine_Feet_pair.Num()-1].Spine_Involved.CachedCompactPoseIndex.GetInt()).ToString());
						...
						//	GEngine->AddOnScreenDebugMessage(-1, 10.1f, FColor::Red, " BONE = "+ owning_skel->GetBoneName(BoneIndex.GetInt()).ToString());

						} while (BoneIndex.GetInt() > 0 && BoneIndex != RootIndex && while_counter < 500);
...
void FAnimNode_DragonSpineSolver::FABRIK_BodySystem(FComponentSpacePoseContext& Output, FBoneReference TipBone, FCSPose<FCompactPose>& MeshBases, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if (spine_Feet_pair.Num() > 0)
	{
		...
		if (is_snake == false || spine_Feet_pair[0].Associated_Feet.Num() > 0)//&& dragon_bone_data.FeetBones.Num()!=0)
		{
			/*
			if (is_single_spine)
			{
		...
		else
		{
			...
			if (Hold_Snake_Root_Bone && owning_skel!= nullptr && Original_AnimatedBoneTransforms.Num() > 1 && test_counter > 20)
			{
				FTransform Original_Root_Bone_Transform = Original_AnimatedBoneTransforms[0].Transform;
				Original_Root_Bone_Transform.SetRotation( Original_Root_Bone_Transform.GetRotation().GetNormalized());


				RootEffectorTransform.SetLocation( (Original_Root_Bone_Transform * owning_skel->GetComponentTransform()).GetLocation() );
			}
		}
	}
	...
	if (spine_Feet_pair.Num() > (spine_hit_pairs.Num() - 1) && spine_Feet_pair.Num() > 1)
	{

		if (is_snake == false || spine_Feet_pair[spine_Feet_pair.Num()-1].Associated_Feet.Num() > 0)//&& dragon_bone_data.FeetBones.Num() != 0)
		{
			/*
			if (is_single_spine)
			{
	...
	for (int i = 0; i < combined_indices.Num(); i++)
	{
		...
		if (Output.Pose.GetPose().IsValidIndex(combined_indices[i]) && i > 0 && i < combined_indices.Num() - 1)
		{
	...
	//output_test.PelvicEffectorTransform.SetRotation(FRotator(0, 0, 0).Quaternion());
	...
	for (int32 LinkIndex = 0; LinkIndex < output_test.NumChainLinks; LinkIndex++)
	{
		...
		FTransform OriginalBone_Transform = MeshBases.GetComponentSpaceTransform(CurrentLink.BoneIndex);
		...
		for (int32 ChildIndex = 0; ChildIndex < NumChildren; ChildIndex++)
		{
			...
			if (LinkIndex+1 < output_test.NumChainLinks)
			{

			//	DragonChainLink const& NextLink = output_test.chain_data[LinkIndex + 1];
				DragonChainLink NextLink = output_test.chain_data[LinkIndex + 1];

				if (complexity_type == ESolverComplexityPluginEnum::VE_Simple)
					NextLink = output_test.chain_data[output_test.chain_data.Num() - 1];

				FTransform NextBoneTransform = output_test.temp_transforms[NextLink.TransformIndex].Transform;
				FTransform Next_OriginalBone_Transform = MeshBases.GetComponentSpaceTransform(NextLink.BoneIndex);
			
				FVector Original_Bone_Direction = (OriginalBone_Transform.GetLocation() - Next_OriginalBone_Transform.GetLocation() ).GetUnsafeNormal();
				FVector Changed_Bone_Direction = (  CurrentBoneTransform.GetLocation() - NextBoneTransform.GetLocation()).GetUnsafeNormal();

			//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, "CurrentLink : " + owning_skel->GetBoneName( CurrentLink.BoneIndex.GetInt()).ToString() + "NextLink : " + owning_skel->GetBoneName(NextLink.BoneIndex.GetInt()).ToString());
				FQuat Current_Next_Rotation_Change = FQuat::FindBetweenNormals(Original_Bone_Direction, Changed_Bone_Direction);

				ChildBoneTransform.SetRotation(Current_Next_Rotation_Change * ChildBoneTransform.GetRotation());

				/*
				TArray<AActor*> test_array;
				FHitResult test_hit;
				UKismetSystemLibrary::SphereTraceSingle(owning_skel->GetWorld(), ( OriginalBone_Transform * owning_skel->GetComponentTransform()).GetLocation(), ( Next_OriginalBone_Transform * owning_skel->GetComponentTransform() ).GetLocation() ,50,ETraceTypeQuery::TraceTypeQuery1,false, test_array,EDrawDebugTrace::ForOneFrame, test_hit,true,FLinearColor::Red);

				UKismetSystemLibrary::SphereTraceSingle(owning_skel->GetWorld(), (CurrentBoneTransform* owning_skel->GetComponentTransform()).GetLocation(), (NextBoneTransform* owning_skel->GetComponentTransform()).GetLocation(), 25, ETraceTypeQuery::TraceTypeQuery1, false, test_array, EDrawDebugTrace::ForOneFrame, test_hit, true, FLinearColor::Yellow);
				*/

			}
			else
			{
				ChildBoneTransform.SetRotation((CurrentBoneTransform.GetRotation() * OriginalBone_Transform.GetRotation().Inverse()) * ChildBoneTransform.GetRotation());
			}
			...
			AnimatedBoneTransforms[CurrentLink.ChildZeroLengthTransformIndices[ChildIndex]].Transform = ChildBoneTransform;
...
void FAnimNode_DragonSpineSolver::Snake_ImpactRotation(int origin_point_index, FTransform& OutputTransform, FCSPose<FCompactPose>& MeshBases)
{
	...
	if (origin_point_index == 0)
	{
		...
		UDragonIK_Library::RectifyNaNRotator(position_Based_rot);
	...
	UDragonIK_Library::RectifyNaNRotator(target_rot);
...
void FAnimNode_DragonSpineSolver::Dragon_ImpactRotation(int origin_point_index, FTransform& OutputTransform, FCSPose<FCompactPose>& MeshBases, bool is_reverse)
{
	...
	FRotator final_rot = FRotator(0,0,0);
...
//FVector character_direction_vector = FVector(0,0,1);
	...
	if (spine_hit_pairs.Num() > origin_point_index - 1)
	{
		...
		if (atleast_one_hit && spine_hit_pairs[origin_point_index].Parent_Spine_Hit.bBlockingHit)
		{
			...
			if (accurate_feet_placement && spine_hit_pairs[origin_point_index].RV_Feet_Hits.Num() > 0)
			{
			...
			if (spine_hit_pairs[opposite_index].RV_Feet_Hits.Num() == 0)
				Feet_Opposite_Mid_Point = owner_skel_w_transform.InverseTransformPosition(spine_hit_pairs[opposite_index].Parent_Spine_Hit.ImpactPoint);
			else
			...
			if (accurate_feet_placement && spine_hit_pairs[opposite_index].RV_Feet_Hits.Num() > 0)
			{
			...
			FVector location_output = FVector(0,0,0);
			...
			if (atleast_one_hit)
			{
				...
				//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, "OG_Cross_Value " + FString::SanitizeFloat( OG_Cross_Value) +" Slanted_Height_Offset "+FString::SanitizeFloat( Slanted_Height_Offset));
...
*/
			...
			FRotator position_Based_rot = FRotator(0,0,0);
			...
			if (origin_point_index == 0)
			{
				...
				if (owning_skel->GetOwner())
				{
					...
					//GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, "Intensifier_side " + FString::SanitizeFloat( Intensifier_side) );
				...
				float is_cross_based_rotation = (Use_Fake_Pelvis_Rotations || is_this_biped) ? -1 :1;

				FVector Forward_Altered_Vector = ((Forward_Direction_Vector*100 + character_direction_vector_CS*-Intensifier_forward*is_cross_based_rotation) - FVector::ZeroVector).GetUnsafeNormal();
				FQuat DiffNormal = FQuat::FindBetweenNormals(( Forward_Direction_Vector ),(Forward_Altered_Vector));

				FVector RightVectorCalculated = FVector::CrossProduct(Forward_Direction_Vector,character_direction_vector_CS);

				FVector Right_Altered_Vector = ((RightVectorCalculated*100 + character_direction_vector_CS*-Intensifier_side*10) - FVector::ZeroVector).GetUnsafeNormal();
				FQuat RightDiffNormal = FQuat::FindBetweenNormals(( RightVectorCalculated ),(Right_Altered_Vector));

				DiffNormal = RightDiffNormal*DiffNormal;
			...
			//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, "RightVectorCalculated : " + (RightVectorCalculated).ToString());
			...
			//	position_Based_rot.Roll = Roll_look_rot.Roll;
				...
				float is_cross_based_rotation = (Use_Fake_Chest_Rotations || is_this_biped) ? -1 :1;

				///
				FVector Forward_Altered_Vector = ((Forward_Direction_Vector*100 + character_direction_vector_CS*-forward_intensity*is_cross_based_rotation) - FVector::ZeroVector).GetUnsafeNormal();
				FQuat DiffNormal = FQuat::FindBetweenNormals(( Forward_Direction_Vector ),(Forward_Altered_Vector));


				FVector RightVectorCalculated = FVector::CrossProduct(Forward_Direction_Vector,character_direction_vector_CS);

				FVector Right_Altered_Vector = ((RightVectorCalculated*100 + character_direction_vector_CS*-right_intensity*10) - FVector::ZeroVector).GetUnsafeNormal();
				FQuat RightDiffNormal = FQuat::FindBetweenNormals(( RightVectorCalculated ),(Right_Altered_Vector));
			...
			UDragonIK_Library::RectifyNaNRotator(current_rot);
...
//			FRotator target_rot = FRotator(UKismetMathLibrary::Lerp<float>( spine_RotDifference[origin_point_index].Pitch, spine_RotDifference[origin_point_index].Pitch,0,adapt), final_rot.Yaw, spine_RotDifference[origin_point_index].Roll).GetNormalized();
		...
		//	target_rot = FRotator(FMath::Sqrt(-1.0f),FMath::Sqrt(-1.0f),FMath::Sqrt(-1.0f));
...
DragonSpineOutput FAnimNode_DragonSpineSolver::DragonSpineProcessor_Snake(FTransform& EffectorTransform, FCSPose<FCompactPose>& MeshBases, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	for (int32 TransformIndex = 1; TransformIndex < NumTransforms; TransformIndex++)
	{
		...
		if (MeshBases.GetPose().IsValidIndex(BoneIndices[TransformIndex]) && MeshBases.GetPose().IsValidIndex(BoneIndices[TransformIndex-1]))
		{
	...
	*/
	...
	//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " Slop x= "+FString::SanitizeFloat(Slop));
		// IF SLOP > PRECISION (EG:- 0.5) THEN PROCEED WITH THE CALCULATION
		if ( TipBoneLinkIndex > 0)
		{
			// Set tip bone at end effector location.
			Chain[TipBoneLinkIndex].Position = CSEffectorLocation;
			
			int32 IterationCount = 0;
			while ( (IterationCount++ < customized_iterations))
			{
				...
				//	GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " CurrentLink.Position = " + CurrentLink.Position.ToString());
...
//				input.temp_transforms[ChainLink.TransformIndex].Transform.SetTranslation(chainTransform.GetLocation());
		...
		*/
		...
		for (int32 LinkIndex = 0; LinkIndex < input.NumChainLinks - 1; LinkIndex++)
		{
			...
			//GEngine->AddOnScreenDebugMessage(-1, 0.01f, FColor::Red, " BONE NAME! " + owning_skel->GetBoneName(CurrentLink.BoneIndex.GetInt()).ToString());
